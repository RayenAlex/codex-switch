...
export const pointDistance = (points: Point[]) => {
  'worklet';
  return points.length < 2 ? 0 : Math.hypot(points[0].x - points[1].x, points[0].y - points[1].y);
};
const midpoint = (points: Point[]) => {
  'worklet';
  return points.length < 2 ? points[0]
...
export function moveImage(input: { before: ImageTransform; start: Point[]; current: Point[] }): ImageTransform {
  ...
  const scale = distance && current.length > 1
    ? clampZoom(before.scale * pointDistance(current) / distance) : before.scale;
