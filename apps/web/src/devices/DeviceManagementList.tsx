...
export function DeviceManagementList({ devices, refreshing, onRefresh, ...cardProps }: DeviceManagementListProps) {
  ...
  return <div className="page-body devices-page">
    <header className="devices-heading"><h1>设备管理</h1>
      <button className="devices-refresh" type="button" disabled={refreshing} onClick={() => void onRefresh()}
        aria-label="刷新设备列表" aria-busy={refreshing}>
        <RefreshCw size={23} className={refreshing ? 'spin' : ''} />
      </button>
    </header>
    <p className="devices-subtitle">查看设备状态，点击卡片切换模型</p>
    <section className="device-overview" aria-label="设备统计">
      <div className="device-stat"><span className="device-stat-icon"><Smartphone size={28} aria-hidden /></span>
        <div><strong>{online}</strong><span><i />当前在线</span></div></div>
      <div className="device-stat"><span className="device-stat-icon"><Layers size={28} aria-hidden /></span>
        <div><strong>{devices.length}</strong><span>全部设备</span></div></div>
    </section>
    <h2 className="devices-list-title">已登录设备 <span>({devices.length})</span></h2>
    {devices.length ? <>
      ...
      <p className="devices-hint"><Info size={23} aria-hidden />
        <span>在线设备暂不支持删除。<br />如需移除，请先在该设备上退出登录。</span></p>
    </> : <div className="devices-empty"><Smartphone size={40} aria-hidden /><h3>暂无设备</h3>
      <p>在电脑上登录同一账号，设备就会自动出现在这里。</p></div>}
  </div>;
}
