...
export function RegistrationSheet({ open, onClose }: RegistrationSheetProps) {
  return <AdaptiveSheet open={open} title="在客户端注册" onClose={onClose} width={440}>
    <div className="registration-guide">
      <p>请下载最新版 Codex Switch 电脑客户端，并在客户端完成注册。</p>
      <p>注册后，即可使用同一账号登录网页版。</p>
      <div className="sheet-actions">
        <a className="registration-download" href={DESKTOP_DOWNLOAD_URL} target="_blank" rel="noopener noreferrer">
          <Download size={18} aria-hidden="true" />下载最新版客户端
        </a>
        <Button block onClick={onClose}>返回登录</Button>
      </div>
    </div>
  </AdaptiveSheet>;
}
