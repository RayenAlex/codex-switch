...
function InfoRow({ label, value, copy = false, secret = false }: {
  ...
  return <div className="account-info-row"><span>{label}</span>
    <strong>{value ? (secret && hidden ? '••••••••' : value) : '未设置'}</strong>
    {secret && value && <button className="icon-button" type="button" onClick={() => setHidden(v => !v)}
      aria-label={hidden ? '显示密码' : '隐藏密码'}>{hidden ? <Eye size={17} /> : <EyeOff size={17} />}</button>}
    {copy && value && <button className="icon-button" type="button" aria-label={`复制${label}`}
...
function AccountTotp({ secret }: { secret: string }) {
  ...
  return <div className="account-info-row"><span>2FA</span>
    <button type="button" className="account-inline-code" disabled={!code} aria-label="复制当前验证码"
      onClick={() => void copyText('验证码', code)}>{secret ? (code || '暂不可用') : '未设置'}
      {code && <Copy size={16} />}</button>
    {code && <small>{30 - Math.floor(now / 1000) % 30} 秒</small>}
  </div>;
}
...
export function AccountInfo({ account, credits, onEdit, onNote, onCredits }: {
  ...
  return <section className="account-detail-section">
    <header><UserRound size={19} /><h3>账号信息</h3><button className="icon-button" type="button"
      aria-label="编辑账号信息" onClick={onEdit}><Pencil size={20} /></button></header>
    <InfoRow label="套餐" value={account.plan || 'ChatGPT'} />
    <InfoRow label="到期时间" value={expiration} />
    {account.expiresAt && expiration !== account.expiresAt && <InfoRow label="预设截止" value={account.expiresAt} />}
    ...
    <button type="button" className="account-info-row" onClick={onNote} aria-label="查看完整备注">
      <span>备注</span><strong className="truncate">{account.note || '未设置'}</strong><ChevronRight size={17} /></button>
    <button type="button" className="account-info-row" onClick={onCredits} aria-label="查看重置卡详情">
      <span>重置卡</span><strong>{credits.loading ? <SpinLoading style={{ '--size': '16px' }} />
        : credits.credits ? `${credits.credits.length} 张` : '暂不可用'}</strong><ChevronRight size={17} /></button>
  </section>;
}
