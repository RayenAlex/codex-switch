...
export function ChatContextSettings({ threadId, api, onClose }: {
  ...
  return <AdaptiveSheet open title="对话上下文设置" width={400} onBack={close} onClose={close}>
    <div className="chat-settings">
      <p className="chat-muted">仅用于当前对话。保存后立即应用；正在回复时会先暂停，修改后自动继续。</p>
      {editor.loading && <p role="status">正在读取上下文设置…</p>}
      {editor.loaded && <>
        <label htmlFor="chat-context-capacity">上下文容量（K Token）</label>
        <input id="chat-context-capacity" className="chat-answer" inputMode="decimal" value={editor.value}
          onChange={event => editor.setValue(event.target.value)} disabled={editor.saving} placeholder="选择或输入容量" />
        ...
        <p className="chat-muted">1 K = 1000 Token；留空使用默认容量。</p>
        <p className="chat-muted">用量会在收到回复后更新。程序会预留部分空间，显示的可用容量可能略小。</p>
        <button type="button" className="chat-button" disabled={editor.saving}
          onClick={() => editor.setValue('')}>恢复默认</button>
      </>}
      {editor.error && <p role="alert" className="chat-error">{editor.error}</p>}
      {!editor.loading && !editor.loaded && <button type="button" className="chat-button"
        onClick={editor.retry}>重试</button>}
      <div className="chat-row"><button type="button" className="chat-button" disabled={editor.saving}
        onClick={close}>取消</button><button type="button" className="chat-button chat-primary"
        disabled={editor.loading || !editor.loaded || editor.saving} onClick={() => { void save(); }}>
        {editor.saving ? '正在保存…' : '保存'}</button></div>
    </div>
  </AdaptiveSheet>;
}
