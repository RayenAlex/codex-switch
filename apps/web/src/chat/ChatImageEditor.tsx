...
export function ChatImageEditor({ image, save, close }: {
  ...
  const html = useMemo(() => imageEditorHtml(image.url), [image.url]);
  ...
  useEffect(() => {
    const receive = (event: MessageEvent) => {
      ...
      try {
        ...
        save(dataUrl);
        close();
      } catch { setError('图片未保存，请减少标注或图片后重试。'); }
    };
  ...
  return <Modal open centered footer={null} title={null} closable={false} maskClosable={false}
    ...
    <iframe ref={frame} title="图片标注" srcDoc={html} sandbox="allow-scripts allow-same-origin" />
