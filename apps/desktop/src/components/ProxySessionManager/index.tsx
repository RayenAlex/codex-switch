  ...
  const columns = useMemo<TableColumnsType<ProxySession>>(() => [
    {
      ...
      width: 140,
      render: (_, session) => (
        <div className={`${styles.sessionCell} ${styles.sessionTitle}`}>
    ...
    },
    {
      ...
      render: (_, session) => {
        ...
        if (used == null) {
          return <span className={styles.muted}>{t("providers.proxy.sessionsContextUnknown")}</span>;
        }
    ...
    },
    {
      ...
      render: (reasoningEffort?: string | null) => (
        reasoningEffort
          ...
          : <span className={styles.muted}>{t("providers.proxy.sessionsRequestUnknown")}</span>
      ),
    },
    {
