...
#[test]
fn missing_usage_count_is_reset_only_after_explicit_acknowledgment() {
    let fixture = Fixture::new();
    ...
    .unwrap();
    assert!(fixture.usage().unconfirmed_requests > 0);
    let mut edit = input("renamed", Some(10.0));
...
fn invalid_prices_keep_reported_tokens_and_require_cost_review() {
    ...
    assert!(usage.unconfirmed_requests > 0);
...
fn failed_writes_are_retained_and_recovered_once_before_admitting_more_usage() {
    ...
    assert!(recovered.unconfirmed_requests > 0);
    assert_eq!(
        fixture.usage().tokens,
        50,
...
#[test]
fn partial_anthropic_streams_are_excluded_even_if_start_event_contains_usage() {
    let fixture = Fixture::new();
    let start = concat!(
    ...
    reader.read_to_end(&mut Vec::new()).unwrap();
    drop(payload);
    assert_eq!(fixture.usage().tokens, 0);
    assert_eq!(fixture.usage().unconfirmed_requests, 0);
}
