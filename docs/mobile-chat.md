...
处理过程汇总为“查看处理过程”，点击后在抽屉中查看。过程说明使用较小的灰色文字；工具活动可继续进入详情抽屉，
返回时回到原处理过程。打开的抽屉会继续接收实时内容。任务计划、本轮修改和报错详情也通过抽屉查看。
...
Android 将文件保存到系统下载文件夹的 `Codex Switch` 目录，保留原文件名；下载显示进度并支持取消。
